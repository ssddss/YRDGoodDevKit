...
@interface MediatorViewController () <UITableViewDelegate, UITableViewDataSource>
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        UIViewController *viewController = [[YBGMediator sharedInstance] YBGMediator_viewControllerForDetail];
    ...
    if (indexPath.row == 2) {
        ...
        [[YBGMediator sharedInstance] YBGMediator_presentImage:[UIImage imageNamed:@"image"]];
    }
    
    if (indexPath.row == 3) {
        // 这种场景下，参数有问题，因此需要在流程中做好处理
        [[YBGMediator sharedInstance] YBGMediator_presentImage:nil];
    }
    
    if (indexPath.row == 4) {
        [[YBGMediator sharedInstance] YBGMediator_showAlertWithMessage:@"casa" cancelAction:nil confirmAction:^(NSDictionary *info) {
            // 做你想做的事
            NSLog(@"%@", info);
        }];
    }
